...
mkdir -p /tmp/ybg_challenge3/public /tmp/ybg_challenge3/private
echo "Welcome to the public directory!" > /tmp/ybg_challenge3/public/welcome.txt
echo "This is a public file." > /tmp/ybg_challenge3/public/public_file.txt

cat > /tmp/ybg_challenge3/private/employee_salaries.txt << EOF
...
cat > /tmp/ybg_challenge3/private/acquisition_plans.txt << EOF
...
cat > /tmp/ybg_challenge3/private/system_credentials.txt << EOF
...
mkdir -p /tmp/ybg_challenge8/sensitive
cat > /tmp/ybg_challenge8/sensitive/credentials.txt << EOF
...
mkdir -p /tmp/ybg_challenge10/config
cat > /tmp/ybg_challenge10/config/system.conf << EOF
...
cat > /tmp/ybg_challenge10/config/tokens.json << EOF
{
...
mkdir -p /tmp/ybg_challenge6/user_uploads

# Challenge 4 setup (rug pull)
mkdir -p /tmp/ybg_challenge4/state
echo '{"weather_tool_calls": 0}' > /tmp/ybg_challenge4/state/state.json
